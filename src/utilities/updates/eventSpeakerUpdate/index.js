...
import { validateData, cleanParams } from './validator';
...
export default function Index({ onClose, initialData, onSuccess, eventId }) {
	...
	const deleteSection = (index) => {
		setState([...state].filter((state, stateIndex) => stateIndex !== index));
	};
	...
	useEffect(() => {
		...
		if (speakers.length) {
			...
			for (let speaker of speakers) {
				const { budget, category, country, description, topic_area } = speaker;
				initialData.push({
	...
	const saveState = () => {
		...
		if (valid) {
			...
			// make patch request
			axios
				.patch(`events/${eventID}/speakers`, { speakers: cleanParams(state), organizer_id: userId })
				.then(() => {
	...
	return (
		<div className='updates sample'>
			<div className='updates__form'>
				<div className='updates__form__header'>Call For Speakers</div>
				{state.map((speakercall, index) => (
					<div className={`updates__form__content ${index !== 0 && '--margin-top'}`}>
						{index === 0 || <img src={deleteSectionIcon} alt='' className='--delete' onClick={() => deleteSection(index)} />}
						...
						<div className='updates__form__content__item'>
							...
							<div className='--singleselect'>
								<Select
									...
									className='--item'
									onChange={(value) => {
										changeListData(index, 'topicArea', value.value);
									}}
									value={
						...
						<div className='updates__form__content__item'>
							<label htmlFor='budget'>Speaker Budget</label>
						...
						<div className='updates__form__content__item updates__form__textareacontent --white'>
							<label htmlFor='budget'>Speaker Description</label>
							<textarea
								name='description'
			...
			<div className='--extra' onClick={addNewCall}>
				<div>
