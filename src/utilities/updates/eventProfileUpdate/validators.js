...
export const validateData = (scheduleState) => {
    return (
        ...
        _localValidator(scheduleState)
    );
}

const _localValidator = (state) => {
    return (
        state.name && state.theme && state.language
    )
}
...
const _validateLocation = (scheduleState) => {
    return (
        (
            scheduleState.location === "Physical" &&
            scheduleState.physicalLink !== ''
        ) ||
        (
            scheduleState.location === "Virtual" &&
            scheduleState.onlineLink.every(o => o !== '')
        ) ||
        (
            scheduleState.location === "Physical and Virtual" &&
            scheduleState.onlineLink.every(o => o !== '') &&
            scheduleState.physicalLink !==''
        )
