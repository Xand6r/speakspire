...
export default function Joinus() {
	...
	return (
		<div className='vision__register'>
			<div className='--heading'>Become an inspiration. Sign up as a speaker.</div>
			<div className='vision__register__benefits'>
				{benefits.map((benefit, i) => (
					<div className='--benefit' key={i}>
						<img src={benefit[0]} alt='' className='--icon' />
						<div className='--text'>{benefit[1]}</div>
					</div>
				))}
			</div>
			<div className='signup' onClick={() => history.push('/category')}>
				Sign Up
			</div>
		</div>
	);
