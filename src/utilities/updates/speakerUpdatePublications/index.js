...
export default function Index({ onClose , onSuccess}) {
	...
	const savePublicationDetails = () => {
		...
		// make patch request
		axios
			.patch(`speakers/${userId}/publications`, {
				publications: cleanData(state),
	...
	return (
		<div className='updates publications'>
			<div className='updates__form'>
				<div className='updates__form__header'>Add Publications</div>
				...
				{state.map((publication, index) => (
					<div key={`${index}-publications`} className='updates__form__content --withmargin'>
						...
						<div className='updates__form__content__item'>
							<label htmlFor='fullname'>Type of Publication</label>
						...
						<div className='updates__form__content__item'>
							<label htmlFor='fullname'>Publication File or Link</label>
							<div
								className={`--two_inputs --half_date ${publication.publicationLink ? '--link' : ''} ${publication.publicationFile ? '--file' : ''}`}>
								<Upload
									{...props}
									beforeUpload={(file) => {
										...
										uploadImage(file)
											.then((res) => {
												changeListData(index, 'publicationFile', res);
											})
									...
									<Button
										...
										{certLoader[index] ? <Spin indicator={antIcon} /> : publication.publicationFile ? 'File uploaded' : 'Upload File'}
									</Button>
								</Upload>
								{publication.publicationFile && (
									<img src={deleteBin} alt='' className='delete' onClick={() => changeListData(index, 'publicationFile', null)} />
								)}
								{!publication.publicationFile && !publication.publicationLink ? <span className='filelinkor'>or</span> : null}
			...
			<div className='updates__action'>
				...
				<div className='save' onClick={savePublicationDetails}>
