...
import { validateData, cleanDate } from './validators';
...
export default function Index({ onClose, initialData, onSuccess, eventId }) {
	...
	const { schedule, name, theme, language } = initialData;
			...
			axios
				.patch(`events/${eventID}/edit`, {
					...
					schedule: {
						...
						date: cleanDate(state.dateFrom, state.dateTo),
						time: cleanDate(state.timeFrom, state.timeTo),
						physicalLink: state.physicalLink,
						onlineLink: state.onlineLink,
						intervals: state.interval,
						interval_day_of_week: state.interval_day_of_week,
					},
				})
				.then(() => {
	...
	useEffect(() => {
		...
		const [{ date, frequency, interval_day_of_week, intervals, onlineLink, physicalLink, time }] = schedule;
	...
	return (
		<div className='updates event_profile'>
			<div className='updates__form'>
				<div className='updates__form__header'>Edit Event</div>

				<div className='updates__form__header__preform'>Event Details</div>
			...
			<div className='updates__form'>
				<div className='updates__form__header__preform'>Event Schedule</div>
				...
				<div className='updates__form__content'>
							...
							<>
								<div className='updates__form__content__item --smallmargin --inputandicon'>
									...
									<input
										...
										required
									/>
									{i !== 0 && <img class='bin' src={binIcon} alt='' onClick={() => deleteLink(i)} />}
								</div>
					...
					{['Recurring Event'].includes(state.frequency) && (
						<>
							{/* ############# recurring event ########## */}
							<div className='updates__form__content__item'>
								<label htmlFor='position'>Interval</label>
							...
							{['Weekly', 'Monthly'].includes(state.interval) && (
								<>
									<div className='updates__form__content__item'>
										<label htmlFor='position'>Day of the Week</label>
